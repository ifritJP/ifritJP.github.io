...
#+BEGIN_SRC lns
macro _name ( declarglist ) {
  { declverlist }
  '{'
     macrostatement
  '}'
   { symbol ope val '{' expandstatement '}' }
 }
}
#+END_SRC
...
次の declarglist は、マクロに与える引数である。
この引数に与える値は、リテラルでなければならない。
ただし、リテラル以外にシンボルを与えることが出来る。

declverlist は、マクロ内で利用する変数を宣言する。

macrostatement は、 declarglist で与えられた値を declverlist に
設定、変更するための処理を記載する。
この処理内で利用可能な関数、制御文は制限される。

次の condexp は、マクロ展開を行なうかどうかの条件判定を行なう式である。
この式が真となる場合、続く expandstatement をそのまま展開する。
この式はシンボル symbol と val の比較演算 ope によって表わす。
この condexp と expandstatement 内で、
declarglist と declverlist を利用することが出来る。


次は、マクロ定義の例である。

#+BEGIN_SRC lns
macro _test( val: int, symbol: Symbol ) {
  let work = 1;
  let funcSym = symbol;  
  {
     work = work + val;
     funcSym = "%sfunc" (symbol);
  }
  if val == 1 {
     funcSym( work );
  }
...
_test( 1, sub ); '' subfunc( 2 );
_test( 2, foo ); '' foofunc( 3 + 2 );
#+END_SRC

上記は、マクロ _test を定義する。
このマクロは 2 つの引数(int,Symbol)を持つ。
declverlist として work と funcSym を定義する。
macrostatement として、 work と funcSym を設定する。
expandstatement として、 funcSym を関数呼び出しする。

結果として、上記マクロは subfunc( 2 ) と foofunc( 3 + 2 ) に展開される。

ここで重要なのは、あくまでマクロはコンパイル時にソースに展開されるのであって、
スクリプト実行時にマクロは動作しない、ということである。

*** マクロの意義

上記の通り、マクロは通常の関数と比べて制限がある。
またマクロで行なえる処理は、マクロでなく関数でも行なうことが可能である。
