...
マクロ内では、次の特殊な syntax を利用できる。

- ~`{ }~
- ,,
- ,,,

macro-statement  内で ~`{}~ を使用すると、
その中で書いたステートメントを、値として扱うことができる。
...
#+BEGIN_SRC lns
macro _test( symbol: str, val: int ) {
   {
       let symbol: sym = ,,,"%sfunc" (str);
       let list: stat[] = [];
       for count = 1, val {
...
ここで注目すべき点は、次の 2 点である。

- symbol 型の変数 sym に、 "%sfunc" (str) で作成した文字列からシンボルを格納した後、
  ,,sym でシンボルを展開している。
- stat[] 型の変数 list に、 ~`{}~ で作成したステートメントを格納した後、
  ,,list でステートメントを展開している。
