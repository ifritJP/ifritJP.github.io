...
#+TITLE: トランスコンパイラ LuneScript 入門 - import/provide 編
...
このように、 import したモジュール内で pub 宣言しているシンボルにアクセス出来ます。
...
なお、アクセス名が同じになるモジュールを import する場合は、
次に説明する as を利用してください。
...
LuneScript で書いたモジュールを require すると、必ずテーブルを返します。
