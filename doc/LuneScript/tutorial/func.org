...
これによって、 sub 内で test が実行され、 foo が出力されます。
...
つまり、 stem! 型の引数、戻り値を持つ関数が form 型に代入可能になり、
非 stem! 型の引数、戻り値を持つ関数は代入不可能になります。

具体的には、次の form1, form3 は代入可能で、form2, form4 は代入不可能です。

#+BEGIN_SRC lns
let form1:form = fn ( val1:stem! ) { };
let form2:form = fn ( val1:stem ) { };
let form3:form = fn ( val1:stem!, val2:stem! ) { };
let form4:form = fn ( val1:stem!, val2:stem ) { };
#+END_SRC
...
これによって、 +全ての+ 関数を form 型として扱うことが出来ますが、
