...
#+BEGIN_SRC bnf
  <comment> ::= "//" <anytoken_br> | "/*" <anytoken> "*/"
     ...
     <code> ::= [<shebang>] [<subfile>] {<lune_control>} {<import>} {<stmt>}

  <shebang> ::= "#!" <token> <anytoken_br>

  <import> ::= "import" [ "go" "/" ] <sym> { <import_dlmt> <sym> } [ "as" <sym> ] ";"
  <import_dlmt> ::= "." | ":"
  ...
  <decl_fn> ::= "fn" [<sym> "." ] <sym> [<generics>] "(" [ <decl_arg_list> ] ")"
		<fn_attrib> [":" <reftype_list>] <block>
  <decl_class> ::= [ "abstract" ] "class" <sym> [<generics>] 
		   [ "extend" [<type>] ["("<type_list> ")" ] ] "{" { <class_field> } "}"
  <decl_proto> ::= "proto" <decl_proto_class_if> <sym> [<generics>] 
  ...
  <if_unwrap> ::= "if" "!" [ "let" <decl_sym_list> "=" ] <exp_list> <block> [ "else" <block> ]
  <sym_list> ::= <sym> { "," <sym> }
  <when> ::= "when" "!" <exp_list> <block> [ "else" <block> ]
  <switch> ::= <switch_keyword> <exp> "{" { "case" <exp_list> <block> }
	       [ "default" <block> ] "}"
  <switch_keyword> ::= "switch" | "_switch"


  <exp_list> ::= <exp> { "," <exp> }
  <match> ::= <match_keyword> <exp> "{" { <match_case> } [ "default" <block> ] "}"
  ...
  <accesser> ::= <accesser_kind> ["&"] [ ":" <reftype> ]
  <accesser_kind> ::= "non" | "pub" | "pri" | "pro" | "local"

  <decl_method_body> ::= <block> | ";"



  <exp> ::= [<op1>] <exp_single> { <op2> <exp> }
  <exp_single> ::= <exp_one> { <exp_suffix> }
  <exp_one> ::= "..." | <omit_enum> | <const_list> | <const_set> | 
		<const_tuple> | <const_map> |
		<paren> | <new> | <literal_int> | <literal_real > |
		<literal_char> | <const_str> | 
		<unwrap> | <anonymous_func> | 
		<literal_bool> | "nil" | "null" | "##" | <reftype> | 
                <stat_block> | <block> | <ref_sym>
  <stat_block> ::= "`{" <stat> "}"
