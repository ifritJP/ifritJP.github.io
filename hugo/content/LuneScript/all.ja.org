...
これは主に go へ変換したときに影響があるもので、
lua への変換を利用している場合は影響ありません。

新しい collection 型の説明をする前に、
...
では、 generics を対応していない go 1.15 、
でどのように LuneScript の generics を実現していたか？というと、

*『go の interface{} 型(現在の any 型)でデータを扱い、*
*アクセスする際に interface{} からキャストして使っていた』* 
...
- &List<str> 型の list から &List<&stem> 型への代入が可能です。
- &List<&stem> 型の list から &List<str> 型へのキャストが可能です。
